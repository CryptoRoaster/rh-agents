...
## Bounded PAPER run (Phase 2N)

`python -m src.runner.main --once` performs exactly one bounded pass — market acquisition where it is switched on, intake, specialist steps, one canonical risk request per case, and a case-bound PAPER fill — and then ends. There is no daemon, scheduler or API-started task, and a pass that produces nothing and says why is a successful pass.

Before releasing a run, `python -m src.runner.main --preflight` reports what this configuration could do. It is a separate mode that cannot be combined with `--once`: it writes nothing, contacts no provider, RPC endpoint, indexer or model, and bounds its two local database reads by the configured step timeout. It distinguishes what was checked locally and met, what was checked locally and is in the way, and what would require an external call and is therefore not claimed either way. Exit `0` means every locally checkable precondition is in place, `2` that something is missing, and `1` that a check could not be carried out. **A green preflight authorises nothing:** the executing mode keeps its own refusals, and every risk, freshness and execution bound still applies.

See [docs/operations-paper-run.md](docs/operations-paper-run.md) for the configuration each capability needs, how to read a preflight, a conservative example run to release separately, what a run can leave behind, and how restarting after an interruption stays safe.
