...
Contract: https://api.geckoterminal.com/docs/v2/swagger.json, inspected 2026-09-10; public V2, Accept version 20230203. Used schemas: networks_list, pool/pool_resource and pools_included. Two reads are performed against them: `/networks/{network}/new_pools` for bounded discovery, and `/networks/{network}/pools/multi/{addresses}` to observe pools this system already holds a case or a position in, addressed by their recorded locators. Both answer with the same pools document, which is why one normalization serves both. Network ID/platform associations were checked against public /networks pages 1 and 3 on that date.
