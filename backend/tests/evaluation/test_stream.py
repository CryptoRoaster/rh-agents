...
# --------------------------------------------------------------------------
# A Codex `error` event: not terminal, and what it is allowed to say
# --------------------------------------------------------------------------


def error_event(message: object) -> dict[str, object]:
    return {"type": "error", "message": message}


def after_error(state: EventAccumulator, message: object) -> tuple[str, ...]:
    """Feed one `error` event after a thread start and return what was kept."""
    feed(state, THREAD, error_event(message))
    return state.safe_error_lines()


def test_an_error_event_does_not_end_the_run() -> None:
    """The regression for the sixth real probe, in one assertion.

    `responses_retry.rs` reports a retryable stream failure through
    `notify_stream_error(..., "Reconnecting... X/Y", ...)` and then carries on.
    The app-server marks it `will_retry: true`; the JSONL processor drops that
    flag, emits a bare `ThreadEvent::Error` and returns `CodexStatus::Running`.
    Treating the event as fatal ended a run the CLI had not given up on.
    """
    state = accumulator()
    feed(state, THREAD, TURN, error_event("Reconnecting... 2/5"), message("final"))
    feed(state, {"type": "turn.completed"})
    assert state.require_consistent_completion() == "final"
    assert state.safe_error_lines() == ("Reconnecting... 2/5",)


def test_an_error_event_changes_nothing_about_the_turn() -> None:
    """It is recorded, and it moves no other state."""
    state = accumulator()
    feed(state, THREAD, TURN, error_event("Reconnecting... 2/5"))
...
def test_a_stream_error_reports_what_codex_said() -> None:
    """Case 1: the message survives, redacted, instead of being dropped.

    The fifth real probe ended on this event and could report nothing but the
    reason code: the message was read, used to pick between two codes, and
    discarded.
    """
    assert after_error(accumulator(), "provider rejected request") == ("provider rejected request",)
...
def test_a_credential_shaped_message_is_dropped_whole(message: str) -> None:
    ...
    fails on the message nobody anticipated. A Codex `error` message is
    typically a single line carrying explanation and value together, which is
    exactly why the whole line goes.
    """
    state = accumulator()
    kept = after_error(state, message)
    assert kept == (REDACTED_LINE,)
    for surface in (" ".join(kept), repr(state.error_lines)):
...
def test_a_token_shaped_value_is_masked() -> None:
    ...
    kept = after_error(accumulator(), f"upstream said {jwt} and gave up")
    assert jwt not in " ".join(kept)
    assert MASKED_VALUE in kept[0]

    opaque = "z" * 48
    other = after_error(accumulator(), f"handle {opaque} expired")
    assert opaque not in " ".join(other)
    assert MASKED_VALUE in other[0]
...
def test_an_absolute_harness_path_becomes_an_alias(tmp_path: Path) -> None:
    ...
    kept = after_error(state, f"could not read {home}/auth.json")
    joined = " ".join(kept)
...
def test_a_broken_redactor_yields_the_marker_and_never_the_message(
    ...
    state = accumulator()
    kept = after_error(state, "a very specific secret detail")
    assert kept == (REDACTION_FAILED,)
    assert "specific secret detail" not in repr(state.error_lines)


def test_a_malformed_error_event_is_still_refused() -> None:
    """An event we cannot read is a contract violation, not a retry notice.

    The loosening is about what an `error` event *means*, not about accepting
    one whose shape is wrong. This stays in the same class as `ITEM_MISSING`.
    """
    state = accumulator()
    with pytest.raises(StreamError) as caught:
        feed(state, THREAD, error_event({"not": "a string"}))
    assert caught.value.failure is EvaluationFailure.PROCESS_FAILED
    assert caught.value.reason_code == "STREAM_ERROR_MALFORMED"
    assert caught.value.safe_lines == ()


def test_a_storm_of_retries_stays_inside_the_diagnostic_budget() -> None:
    """A run that reconnects many times must not grow this without limit.

    The most recent lines are the ones kept: a turn that failed after five
    attempts is explained by what it said last.
    """
    state = accumulator()
    feed(state, THREAD, TURN)
    for index in range(40):
        feed(state, error_event(f"Reconnecting... {index}/40"))
    kept = state.safe_error_lines()
    assert len(kept) == MAX_LINES
    assert kept[-1] == "Reconnecting... 39/40"


def test_a_failed_turn_carries_the_retries_that_led_to_it() -> None:
    """The terminal signal explains itself with what came before it."""
    state = accumulator()
    feed(state, THREAD, TURN, error_event("Reconnecting... 2/5"))
    with pytest.raises(StreamError) as caught:
        feed(state, {"type": "turn.failed", "error": {"message": "giving up"}})
    assert caught.value.failure is EvaluationFailure.TURN_FAILED
    assert caught.value.safe_lines == ("Reconnecting... 2/5", "giving up")


def test_a_failed_turn_redacts_its_own_message_too() -> None:
    """`turn.failed` carries a message, and it goes through the same filter."""
    state = accumulator()
    feed(state, THREAD, TURN)
    with pytest.raises(StreamError) as caught:
        feed(state, {"type": "turn.failed", "error": {"message": "Bearer SUPER_SECRET"}})
    assert caught.value.safe_lines == (REDACTED_LINE,)
    assert "SUPER_SECRET" not in repr(caught.value)


def test_the_state_before_the_error_is_still_readable() -> None:
    """Cases 6 and 9: no turn yet, a thread id, and no tool activity."""
    state = accumulator()
    after_error(state, "boom")
    assert state.turn_started is False
    assert state.thread_id == "t-1"
    assert state.observed_tool_activity() == ()
...
def test_a_turn_that_had_started_is_visible_after_the_error() -> None:
    ...
    state = accumulator()
    feed(state, THREAD, TURN, error_event("boom"))
    assert state.turn_started is True
    assert state.thread_id == "t-1"


def test_tool_activity_seen_before_the_error_is_still_counted() -> None:
    """Case 8: observation up to the error, which is not the same as none."""
    state = accumulator()
    feed(
        state,
        THREAD,
