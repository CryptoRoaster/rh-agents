...
ATLAS receives `{lease, context, submit}` and no RPC, indexer, session, HTTP client, signer, wallet, executor or ledger write; the collector uses approved services internally. The RPC client gained three explicit read methods behind its allowlist, with no generic call path. Holder concentration and contract creator have no verified provider for either chain and are reported UNAVAILABLE, which means ATLAS cannot reach CLEAR in a real deployment today — the intended fail-closed outcome rather than a gap to work around. Phase 2D adds no migration. See [Phase 2D ATLAS details](docs/phase-2d.md).
