...
A ticker is not an identity: a bare symbol can never bind an observation to a TradeCase, an exact contract address is re-checked against this token and this chain rather than believed, and the policy structurally refuses to admit an ambiguous reference. The model's social demand indication is capped by the measured breadth of authorship, it may only cite posts it was actually shown, and its output schema has no field in which a side, a size or an approval could be expressed — so an injected instruction has nothing to aim at. `signal-quality-v1` and `signal-v1` are versioned; evidence stores metrics, hashes and identifiers rather than third-party post text.

SIGNAL remains required and not safety-critical, exactly as the workflow already said: an unusable social set leaves the requirement pending rather than blocking a case, a negative reading stops nothing, and sentiment stays out of the risk digest. No social provider is integrated — the candidates are researched in the phase document, and scraping around an access control is not treated as a fallback. Phase 2F adds no migration and starts no worker. See [Phase 2F SIGNAL details](docs/phase-2f.md).
